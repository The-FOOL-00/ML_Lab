...
reduction is driven by the data rather than an arbitrary number.
...
\section{Conclusion}
Across ten classifiers evaluated with and without PCA on the Spambase dataset, PCA provided
no net benefit here: it reduced 5-fold cross-validated accuracy for all ten models and test
accuracy for eight of the ten, with the two exceptions (SVM, Logistic Regression) improving
only marginally and only on the single test split, not in cross-validation. The clearest
pattern is architectural rather than dataset-specific: linear and distance-based models
(SVM, KNN, Logistic Regression) are largely indifferent to an orthogonal rotation of the
feature space, while tree-based and ensemble models lose real predictive power when their
axis-aligned splits act on rotated components instead of the original, individually
meaningful word- and character-frequency features. The best overall models were XGBoost and
Random Forest without PCA (95.5\% and 95.3\% cross-validated accuracy respectively); PCA
would only be worth revisiting here if paired with a much more aggressive component count
(well below the 48 needed for 95\% variance) or applied to a dataset with a less favourable
sample-to-feature ratio than this one's.
